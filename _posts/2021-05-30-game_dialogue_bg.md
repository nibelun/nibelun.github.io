...
因此在绘制之前我们需要先知道各个部分的宽高占比。

```
对话框宽=角落宽度 * 2 + 文字宽度
对话框高=角落高度 * 2 + 文字行数 * 字体大小
```

可以使用`string_width`直接获取某个字符串总长度，使用这个函数要注意，不同的字体、字号会影响其结果，并且中文与英文的宽度也会受到字体影响。
若文本框有宽度限制和高度限制，则需要另外实现**横向自动换行**和**纵向滚动**能力。（看样子前面的代码就避免不了要升级一次了）
...
P.S.：有一些坑，没踩过是很难发现的，比如GMS使用的十六进制并非RGB，然而`maker_color_rgb`的入参顺序是RGB。
这种前后风格不一致实在是特别容易困扰……
![2021053003](/assets/img/post/2021053003.png)
![2021053004](/assets/img/post/2021053004.png)
![2021053005](/assets/img/post/2021053005.png)
![2021053006](/assets/img/post/2021053006.png)

对话框绘制完毕以后就是文本部分了。
文本部分相对复杂，换行部分的XY逻辑计算有点混乱，建议用一张笔和纸做草稿一边画图一边写代码……
